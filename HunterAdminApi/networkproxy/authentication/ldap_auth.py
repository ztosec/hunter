...
class NetWorkProxyLdapAuth:
    """
    网络代理LDAP认证模式
    if not http_until.is_white_hosts(ngx_get_headers(), conf) and not conf.anonymous
    """
    ...
    def auth_login_logic(username, password):
        ...
        """
        try:
            req = requests.post(url="http://127.0.0.1:8888/api/v1/ldap/login/",
