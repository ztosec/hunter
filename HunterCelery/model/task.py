...
class TaskService:
    ...
    @staticmethod
    @MysqlManage.close_database
    def get_tasks_url_vuln_num(user_id):
        """
        SELECT task.id, task.created_time,task.task_name, (SELECT COUNT(*) FROM url WHERE url.task_id=task.id) AS url_num,(SELECT COUNT(*) FROM vulnerability WHERE (vulnerability.task_id = task.id)) AS vul_total_num  FROM task INNER JOIN usertask ON usertask.task_id = task.id WHERE	usertask.user_id='1';
        获取任务详情和url,vlun数量
        :param user_id: 
        :return: 
        """
        ...
        from model.user_task import UserTask
        tasks = Task.select(Task.id, Task.task_name, Task.created_time, Task.task_status,
                            Url.select(fn.COUNT(Url.id)).alias('urls_num').where(Url.task_id == Task.id),
                            Url.select(fn.COUNT(Url.id)).alias('unscaned_urls_num').where(Url.task_id == Task.id, Url.status != TaskStatus.DONE),
                            Url.select(fn.COUNT(Url.id)).alias('scaned_urls_num').where(Url.task_id == Task.id, Url.status == TaskStatus.DONE),
                            Vulnerability.select(fn.COUNT(Vulnerability.id)).alias('vulns_num').where(
                                Vulnerability.task_id == Task.id),
                            Vulnerability.select(fn.COUNT(Vulnerability.id)).alias('middle_vulns_num').where(
                                Vulnerability.task_id == Task.id, Vulnerability.level == "middle"),
                            Vulnerability.select(fn.COUNT(Vulnerability.id)).alias('high_vulns_num').where(
                                Vulnerability.task_id == Task.id, Vulnerability.level == "high"),
                            Vulnerability.select(fn.COUNT(Vulnerability.id)).alias('low_vulns_num').where(
                                Vulnerability.task_id == Task.id, Vulnerability.level == "low")).join(UserTask,
                                                                                                      JOIN.INNER, on=(
            UserTask.task_id == Task.id)).where(UserTask.user_id == user_id).execute()
