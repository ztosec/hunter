...
class BaseModelService(object):
    ...
    def get_objects(cls, **kwargs):
        ...
        """
        objects = list()
        for object in BaseModelService.get_fields_by_where(cls, **kwargs):
    ...
    def get_fields_by_where(cls, **kwargs):
        ...
        基础的CURD操作，通用类
        To use:
        >>> tasks = BaseModelService.__get_fields_by_where(fields=(Task.id, Task.hunter_status), where=(Task.id > 1))
        >>> print(tasks)
...
class OrmModelJsonSerializer:
    ...
    def serializer(instances):
        ...
        :return: 
        """
        if not isinstance(instances, list) and isinstance(instances, BaseModel) and hasattr(instances, '__str__'):
